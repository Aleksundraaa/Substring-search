...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Инструмент для тестирования производительности обработки данных. '
    )
    parser.add_argument(
        '--sizes', type=int, nargs='+',
        default=[1024, 10240, 102400, 1048576, 10485760],
        help='Список размеров данных для тестирования байтах. '
             'По умолчанию используются размеры: [1024, 10240, 102400, 1048576, 10485760]. '
             'Вы можете указать несколько значений, разделённых пробелами без скобок.'
    )
    parser.add_argument(
        '--pattern', type=str, default='pattern',
        help='Шаблон, используемый для генерации данных. '
             'По умолчанию используется шаблон "pattern". '
             'Вы можете указать свой собственный шаблон, указать без кавычек.'
    )
    parser.add_argument(
        '--batch', action='store_true',
        help='Запуск в пакетном режиме. '
             'При активации этого параметра результаты будут отображены только после завершения всех тестов.'
    )
