...
class TimeResultsPlotter:
    def __init__(self, filename='time_result.txt'):
    ...
    def read_data(self):
        ...
        with open(file_path, 'r') as file:
            next(file)
            for line in file:
                data_type, size, algotithm, execution_time = line.split()
                size = int(size)
                execution_time = float(execution_time)

                if algotithm in self.results:
                    self.results[algotithm][data_type].append((size, execution_time))
    ...
    def plot_results(self, batch_mode=False):
        ...
        for algorithm, data_types in self.results.items():
            for data_type, size_time in data_types.items():
                if size_time:
                    size, time = zip(*size_time)
                    plt.plot(size, time, marker='o', label=f'{algorithm} - {data_type}')

        plt.xscale('log')
        plt.yscale('log')
        plt.title('Время выполнения алгоритмов по типу данных')
        ...
        plt.savefig(os.path.join(image_dir, 'time_result_plot.png'))
